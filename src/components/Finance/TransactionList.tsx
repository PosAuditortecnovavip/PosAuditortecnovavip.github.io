          ...
          <div key={t.id} className="bg-surface/30 rounded-lg p-4 text-sm space-y-2 flex justify-between items-start gap-3">
            <div className="flex-1 min-w-0">
              <div className="flex items-center gap-2">
                {t.type === 'income' ? <ArrowUpCircle size={20} className="text-success shrink-0" /> : <ArrowDownCircle size={20} className="text-danger shrink-0" />}
            ...
            <div className="text-right shrink-0 ml-2">
              ...
              >
                <Download size={18} />
                <span className="hidden md:inline text-sm">Recibo</span>
              </button>
            </div>
          </div>
        ))
